...
bot.on('message', async message => {
  ...
  connection.query(`SELECT prefix FROM prefixes WHERE guildID = '${message.guild.id}'`, function(err, result) {
    ...
    if (err) console.log(err);
    console.log("Test2");
    if (!result) connection.query(`INSERT INTO prefixes (guildID, prefix) VALUES ('${message.guild.id}', '${botConf.defaultPrefix}')`, function(err, result){
      console.log("Test3");
      if (err) console.log(err);
      console.log("Test4");
  ...
  if (message.content.startsWith(useprefix)) {
    ...
    connection.query(`SELECT coins FROM money WHERE guildID = '${message.guild.id}' AND userID = '${message.author.id}'`, function(err, result) {
      if (err) console.log(err);
      if (!result) connection.query(`INSERT INTO money (guildID, guildName, userID, userName, coins) VALUES '${message.guild.id}', '${message.guild.name}', '${message.author.id}', '${message.author.username}', '${coinstoadd}'`, function(err, result) {
        if (err) console.log(err);
      });
