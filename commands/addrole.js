...
module.exports.run = async (bot, message, args) => {
    ...
    if(roleMember.roles.has(gRole.id)) return message.reply("That user already has that role.");
    await(roleMember.addRole(gRole.id));

    try{
        await roleMember.send(`You have been assigned the role ${gRole.name} in the server ${message.guild.name}.`)
    }catch(e) {
        message.channel.send(`<@${roleMember.id}> has been assigned the role ${gRole.name}.`)
    };
