...
module.exports.run = async (bot, message, args, connection, useprefix) => {
    ...
    let user = message.guild.member(message.mentions.members.first() || message.guild.members.cache.get(s1) || message.guild.members.cache.find(u => u.name === s1));
    let role = message.guild.role(message.mentions.roles.first() || message.guild.roles.cache.get(s2) || message.guild.roles.cache.find(r => r.name === s2));
