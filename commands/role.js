...
module.exports.run = async (bot, message, args, connection, useprefix) => {
    ...
    if (operation.toLowerCase() != "add" || operation.toLowerCase() != "remove") return message.channel.send("Please only specify valid command operations. (add/remove)").then(msg => msg.delete({timeout: 3000})).catch(err => console.error(err));
