module.exports = {
    ...
    async execute(Discord, bot, connection, message, args, useprefix) {
        ...
        if (!message.member.hasPermission("ADMINISTRATOR")) return message.channel.send("You do not have the required permissions to run this command.").then(msg => msg.delete({timeout: 3000})).catch(err => console.error(err));
        ...
        if (operation == "add") {
            ...
            message.channel.send(`You successfully added ${user.user.username} to the ${role.name} role!`).then(msg => msg.delete({timeout: 3000})).catch(err => console.error(err));
        } else if (operation == "remove") {
            await user.roles.remove(role.id).catch(err => console.error(err));
            message.channel.send(`You successfully removed ${user.user.username} from the ${role.name} role!`).then(msg => msg.delete({timeout: 3000})).catch(err => console.error(err));
        } else {
            return;
        };
    },
